...
if($kategori == 'Semua') {
    $query = "SELECT * FROM produk_umkm ORDER BY id_produk DESC";
} else {
    $query = "SELECT * FROM produk_umkm WHERE kategori_umkm='".mysqli_real_escape_string($conn, $kategori)."' ORDER BY id_produk DESC";
}
...
<title>Produk UMKM - Explore Lamsel</title>
...
<section class="pt-36 pb-20 max-w-7xl mx-auto px-6">
  <h2 class="text-3xl font-bold mb-10 text-center text-gray-800">
    Kategori <span class="text-blue-700"><?= htmlspecialchars($kategori) ?></span>
  </h2>

  <div class="flex flex-col md:flex-row gap-10">
    <!-- Sidebar Kategori -->
    <aside class="w-full md:w-1/4 bg-white rounded-xl shadow p-6">
      <h3 class="text-gray-700 text-sm font-semibold mb-3 uppercase tracking-wide">Kategori Produk</h3>
      <ul class="space-y-2 text-gray-600 text-sm">
        <?php
        $categories = ['Semua','Agribisnis','Fashion','Kerajinan Tangan','Kopi','Oleh-Oleh','Tenun & Songket'];
        foreach($categories as $cat) {
            $active = ($kategori == $cat) ? 'text-blue-700 font-semibold' : 'hover:text-blue-600';
            echo '<li><a href="produk.php?kategori='.urlencode($cat).'" class="'.$active.'">› '.$cat.'</a></li>';
        }
        ?>
      </ul>
    </aside>
  ...
  if(mysqli_num_rows($result) > 0) {
      while($row = mysqli_fetch_assoc($result)) {
          ...
          <div class="bg-white rounded-xl shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden">
            <img src="'.$imgPath.'" alt="'.htmlspecialchars($row['produk_umkm']).'" class="w-full h-56 object-cover">
            <div class="p-5">
              <h3 class="font-semibold text-gray-800 text-lg mb-1">'.htmlspecialchars($row['produk_umkm']).'</h3>
              <p class="text-gray-600 text-sm">Rp '.number_format($row['harga_umkm'],0,',','.').'</p>';
          if(!empty($row['penjual_umkm'])) {
              echo '<p class="text-xs text-gray-400 mt-1">'.htmlspecialchars($row['penjual_umkm']).'</p>';
          }
          if(!empty($row['stok_umkm'])) {
              echo '<p class="text-gray-500 text-sm mt-1">Stok: '.intval($row['stok_umkm']).'</p>';
          }
          echo '
              <a href="detail_produk.php?id='.$row['id_produk'].'" 
                 class="mt-3 w-full block text-center bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-md text-sm font-medium transition">
                 BELI SEKARANG
              </a>
            </div>
          </div>';
      ...
      echo '<p class="text-gray-500 italic text-center py-10">Belum ada produk di kategori ini.</p>';
  }
  ?>
</div>
</section>
