...
<!-- 🤍 HERO SECTION - FOTO MELENGKUNG POLOS (Versi Proporsional) -->
<section class="relative flex items-center justify-center pt-28 pb-6 bg-white">
  <div class="max-w-3xl w-full flex justify-center">
    <!-- Bingkai melengkung -->
    <div class="overflow-hidden rounded-[40px] shadow-md bg-white">
      <img 
        src="img/bupati.png" 
        alt="Explore Banner"
...
<section class="bg-white pt-32 pb-20">
  <div class="max-w-6xl mx-auto px-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-10">
      <div>
        <h2 class="text-3xl md:text-4xl font-bold text-purple-900 mb-2">Our Destinations</h2>
        <div class="border-b border-gray-200"></div>
      </div>
      <a href="tambah_destination.php"
         class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-sm font-semibold shadow-md transition-all mt-4 sm:mt-0">
        + Tambah Destination
      </a>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
      <?php
      $query = "SELECT * FROM destination WHERE LOWER(kategori) != 'top destination' ORDER BY id DESC";
      $result = mysqli_query($conn, $query);

      if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
          $gambar = !empty($row['gambar']) 
            ? (strpos($row['gambar'], 'uploads/') === 0 ? $row['gambar'] : 'uploads/' . $row['gambar'])
            : 'img/default.jpg';
          ...
          <a href="detail_destination.php?id=' . $row['id'] . '" 
             class="rounded-2xl overflow-hidden shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 bg-white">
            <div class="w-full h-[260px] overflow-hidden bg-gray-100">
              <img src="' . $gambar . '" alt="' . $nama . '" 
                   class="w-full h-full object-cover object-center transition-transform duration-500 hover:scale-105">
            </div>
            <div class="p-5 pb-7">
              <h3 class="font-bold text-gray-900 text-base uppercase mb-2 leading-snug tracking-wide">'
                . strtoupper($nama) . '
              </h3>
                ...
                <span class="flex items-center space-x-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                          d="M17.657 16.657L13.414 12.414a4 4 0 10-5.657 5.657l4.243 4.243a8 8 0 111.414-1.414z" />
                  </svg>
                  <span>' . $kategori . '</span>
                </span>
              </div>
              <p class="text-gray-500 text-sm mt-2 line-clamp-2" title="' . $alamat . '">
                ' . $alamat . '
              </p>
            </div>
          </a>';
        ...
        echo '
        <div class="text-center text-gray-500 italic py-10 w-full">
