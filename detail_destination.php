...
$data = [
  'nama' => 'Kenali Kopi',
  'deskripsi' => 'Kenali Kopi adalah salah satu tempat nongkrong hits di Lampung Barat yang menawarkan berbagai varian minuman kopi kekinian...',
  'gambar' => 'img/kenali_kopi.png',
  'kategori' => 'Cafe',
  'alamat' => 'Jalan Raden Intan II, Liwa – Lampung Barat',
  'jam_buka' => 'Senin–Jumat 10.00–20.00 | Sabtu–Minggu 10.00–22.00'
];

// Ambil data berdasarkan ID
$id = isset($_GET['id']) ? intval($_GET['id']) : 0;
if ($id > 0) {
  $query = mysqli_query($conn, "SELECT * FROM destination WHERE id = $id LIMIT 1");
...
$gambar_src = 'img/default.jpg';
if (!empty($data['gambar'])) {
  $g = $data['gambar'];
  if (preg_match('/^https?:\/\//', $g)) $gambar_src = $g;
  elseif (strpos($g, 'uploads/') === 0 || strpos($g, 'img/') === 0) $gambar_src = $g;
  else $gambar_src = 'uploads/' . $g;
}
?>
<!DOCTYPE html>
<html lang="id">
<head>
  ...
  <title><?= htmlspecialchars($data['nama']) ?> - Explore Lamsel</title>
  <title>Our Destinations - ExploreLamsel</title>
  ...
  <style>
    ...
    .gold-gradient {
      background: linear-gradient(90deg, #c4a36e, #e1c38b);
...
<!-- 🔵 NAVBAR UNTUK HALAMAN DESTINATION -->
  ...
  <div class="max-w-7xl mx-auto px-6 py-3">
    <div class="flex justify-between items-center bg-white rounded-xl shadow-md px-5 py-2.5 relative">
      ...
      <nav class="hidden md:flex space-x-6 text-gray-800 text-sm font-medium tracking-wide relative">
        ...
        <a href="destination.php" class="relative text-blue-600 font-semibold after:content-[''] after:absolute after:-bottom-1 after:left-0 after:w-full after:h-[2px] after:bg-blue-500">DESTINATION</a>
...
<style>
  * {
  ...
  /* Supaya link aktif (DESTINATION) tetap stabil tidak ikut animasi hover */
...
<!-- 💫 DETAIL SECTION -->
<section class="pt-36 pb-20 bg-gradient-to-b from-white to-gray-50">
  <div class="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center">
    <!-- 📸 Gambar -->
    <div class="flex justify-center" data-aos="zoom-in">
      <img src="<?= htmlspecialchars($gambar_src) ?>" alt="<?= htmlspecialchars($data['nama']) ?>"
           class="rounded-3xl shadow-xl hover-zoom w-full md:w-11/12 border border-gray-100">
    </div>

    <!-- 🧾 Detail -->
    <div class="space-y-6" data-aos="fade-left">
      <h2 class="text-4xl font-bold gold-gradient"><?= htmlspecialchars($data['nama']) ?></h2>
      <p class="text-gray-600 leading-relaxed text-justify"><?= nl2br(htmlspecialchars($data['deskripsi'])) ?></p>

      <div class="space-y-2 text-sm text-gray-700">
        <p><span class="font-semibold text-gray-800">📍 Alamat:</span> <?= htmlspecialchars($data['alamat']) ?></p>
        <p><span class="font-semibold text-gray-800">🕒 Jam Buka:</span> <?= htmlspecialchars($data['jam_buka']) ?></p>
        <p><span class="font-semibold text-gray-800">🏷️ Kategori:</span> <?= htmlspecialchars($data['kategori']) ?></p>
      </div>

      <a href="destination.php" 
         class="inline-block mt-5 bg-gradient-to-r from-blue-500 to-blue-700 text-white px-6 py-2.5 rounded-xl font-semibold text-sm shadow-md hover:shadow-xl hover:scale-105 transition">
...
<section class="bg-gradient-to-b from-white to-gray-50 py-16">
  <div class="max-w-6xl mx-auto px-6">
    <h2 class="text-3xl font-semibold mb-10 text-center gold-gradient" data-aos="fade-up">Destinasi Lainnya</h2>

    <div class="grid sm:grid-cols-2 md:grid-cols-4 gap-8" data-aos="fade-up" data-aos-delay="100">
      <?php
      $other_query = mysqli_query($conn, "SELECT * FROM destination WHERE id != $id ORDER BY RAND() LIMIT 4");
      if ($other_query && mysqli_num_rows($other_query) > 0) {
        while ($row = mysqli_fetch_assoc($other_query)) {
          $gambar = !empty($row['gambar']) ? $row['gambar'] : 'img/default.jpg';
          $thumb = (preg_match('/^https?:\/\//', $gambar)) ? $gambar : 
                   ((strpos($gambar, 'uploads/') === 0 || strpos($gambar, 'img/') === 0) ? $gambar : 'uploads/'.$gambar);
      ?>
      <a href="detail_destination.php?id=<?= $row['id'] ?>" 
         class="bg-white rounded-3xl shadow-md hover:shadow-2xl overflow-hidden transform hover:-translate-y-2 transition-all duration-500 group">
        <div class="relative h-48 overflow-hidden">
          <img src="<?= htmlspecialchars($thumb) ?>" alt="<?= htmlspecialchars($row['nama']) ?>" 
          ...
          <div class="absolute bottom-3 left-3 text-white text-sm font-medium bg-black/40 px-3 py-1 rounded-full backdrop-blur-sm">
            <?= htmlspecialchars($row['kategori']) ?>
          </div>
        </div>
        <div class="p-5">
          <h3 class="text-lg font-semibold text-gray-800 mb-1 truncate"><?= htmlspecialchars($row['nama']) ?></h3>
          <p class="text-sm text-gray-500 truncate"><?= htmlspecialchars($row['alamat']) ?></p>
        </div>
      </a>
      <?php
        }
      } else {
        echo '<p class="text-gray-500 text-center col-span-4 italic">Belum ada destinasi lainnya 🌿</p>';
      }
      ?>
    </div>
  </div>
...
<head>
  ...
  <title>Footer — ExploreLamsel</title>
  <style>
    :root {
      --red: #e74c3c;
      --muted: #8a8f98;
